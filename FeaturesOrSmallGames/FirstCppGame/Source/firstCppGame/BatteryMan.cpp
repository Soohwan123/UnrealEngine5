...
ABatteryMan::ABatteryMan()
{
	...
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom")); // How are we gonna name it on the unreal engine component list
	CameraBoom->SetupAttachment(RootComponent);

	CameraBoom->TargetArmLength = 300.0f; // How far the camera is going to be away from the character
...
void ABatteryMan::Tick(float DeltaTime)
{
	...
	if (Power <= 0) {
		if (!bDead) {
			...
			// false means it doesn't repeat every 3 seconds(3.0f) if true it repeats every 3 secs
		}
	}
}
